...
| 模式 | 触发字段 | 适用 | 结果形态 |
|---|---|---|---|
| 求和模式 | 默认 | SAS / PHQ-9 / GAD-7 / PSS-10 / GSES / PSSS / HCL-32 / UCLA | 总分 + 区间判级 + 分维度条 |
| 极性模式 | `poleMode` | 十六型人格 | 类型代码 + 四维双极条 |
| 维度均值 | `dimsMode` | 大五人格 / 家庭环境FES / 父母教养EMBU | 多维均值（1-5）+ 档位解读 + 组合提示 |
...
`node --check` 全部通过；引擎冒烟测试 14/14（含 SAS 满分/零分、PHQ-9 第 9 题强制转介、PSS-10 双向极值、PSSS 分维度、HCL-32 阈值边界、UCLA 双向极值、FES/EMBU 三维均值与组合提示）。
